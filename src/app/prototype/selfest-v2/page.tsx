...
const FAQS = [
  {
    ...
    answer:
      '패브릭씰러는 프리미엄 소재로 외풍·소음 개선에 더 유리하고 먼지날림이 없습니다. 일반 모헤어는 시간이 지나면 먼지날릴 우려가 있지만 비용 부담을 낮출 수 있는 선택지입니다. 일반적으로 반려동물 또는 어린 아이가 있는 가정에서는 패브릭씰러 시공을 더 선호하십니다.',
  },
  {
    question: '창틀 레일 모헤어까지 교체하면 금액이 달라지나요?',
    answer:
      '네. 창틀 레일 모헤어까지 교체하면 작업량이 늘어나 예상 금액이 달라집니다. 다만 고객이 직접 구분하기 어려운 경우가 많아, 상담 과정이나 현장에서 함께 확인해 정확히 안내드립니다.',
  },
  {
    question: '인테리어 공사 중이라면 모헤어 교체 작업은 언제 하는 게 좋을까요?',
    answer:
      '가급적 다른 인테리어 일정보다 앞서 진행하시게 되면 보양 비용을 절감하고, 시공 중 오염이나 찍힘에 대한 염려를 줄이실 수 있습니다. 하지만 다른 공사가 선행된다 하더라도 프리미엄 보양을 통해 깔끔하고 안전한 시공을 해드리니 걱정하지 않으셔도 됩니다.',
  },
];
...
export default function SelfEstimateV2PrototypePage() {
  ...
  const selectedTotal = selectedPlan && protectionOption ? totals[selectedPlan] : null;
          ...
          {step === 2 && smsSent && (
            <section className="space-y-6">
              ...
              <div className="text-center text-sm">
                <button onClick={() => setStep(3)} className="font-semibold text-[#b10000]">
                  프로토타입에서 다음 단계 보기
                </button>
              </div>
            </section>
            ...
            <section className="space-y-5">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() => setProtectionOption('none')}
                  ...
                  <button
                    onClick={() => setProtectionOption('premium')}
                    className={`border p-4 text-left ${
                ...
                {!protectionOption && (
                  <p className="text-xs text-[#6a6a6a]">집 상황에 맞는 보양 방식을 선택해주세요.</p>
                )}
              </div>
              )}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() => setIncludeRailMohair(false)}
                    className={`border p-4 text-left ${
                      !includeRailMohair
                        ? 'border-[#b10000] bg-[#fff1ee]'
                        : 'border-[#dddddd] bg-white'
                  ...
                  <button
                    onClick={() => setIncludeRailMohair(true)}
                    className={`border p-4 text-left ${
                      includeRailMohair
                        ? 'border-[#b10000] bg-[#fff1ee]'
                        : 'border-[#dddddd] bg-white'
